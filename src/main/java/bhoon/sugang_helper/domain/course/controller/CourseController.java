...
public class CourseController {
    ...
    @Operation(summary = "단과대 및 학과 계층 구조 조회", description = "단과대별 학과 목록을 트리 구조로 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "계층 구조 조회 성공", content = @Content(schema = @Schema(implementation = CommonResponse.class)))
    })
    @GetMapping("/departments/hierarchy")
    public ResponseEntity<CommonResponse<List<CollegeHierarchyResponse>>> getDepartmentHierarchy() {
        List<CollegeHierarchyResponse> hierarchy = courseService.getCollegeHierarchy();
        return CommonResponse.ok(hierarchy, "단과대 및 학과 계층 구조 목록입니다.");
    }
