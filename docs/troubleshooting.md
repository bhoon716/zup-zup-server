...
## 26. 대규모 데이터 업데이트 시 MySQL Lock Wait Timeout 발생

### 문제 상황

대용량 XML 파싱 및 전체 과목 크롤링(`CourseCrawlerService`) 작업 중, 특정 시점에 `com.mysql.cj.jdbc.exceptions.MySQLTransactionRollbackException: Lock wait timeout exceeded` 에러가 발생하며 트랜잭션이 롤백되었습니다.

### 원인 분석

1. **대량의 동시 쓰기**: 약 5,500건의 강의 데이터를 한 번의 트랜잭션 내에서 업데이트하거나, 동시에 여러 스레드가 동일한 테이블의 인덱스에 접근하면서 데이터베이스 잠금(Lock) 경합이 발생했습니다.
2. **긴 트랜잭션 유지**: 크롤링 후 변경 사항을 감지하고 알림을 발송하는 로직이 하나의 트랜잭션 범위 내에 묶여 있을 경우, 잠금을 보유하는 시간이 MySQL의 `innodb_lock_wait_timeout`(기본 50초)을 초과하게 됩니다.

### 대응 및 의사결정

- **현상 관찰**: 현재 한 번만 발생한 간헐적 이슈로 판단하여, 즉각적인 로직 수정보다는 문서화 후 모니터링을 우선하기로 결정했습니다.
- **향후 개선 방안**:
  - **배치 처리**: 수천 건의 데이터를 한 번에 저장하지 않고 100~500건 단위로 트랜잭션을 분리하여 잠금 점유 시간을 단축합니다.
  - **트랜잭션 범위 축소**: DB 저장 로직과 알림 발송 로직을 분리하여 각 트랜잭션의 길이를 최소화합니다.

### 결과

문서화를 통해 이슈의 잠재적 원인을 기록함으로써, 향후 재발 시 신속한 대응(Batching 로직 도입 등)을 위한 근거를 마련했습니다.

불규칙한 텍스트 형식을 가진 다양한 과목들의 시간표 정보를 정확하게 엔티티화하여 저장할 수 있게 되었습니다.
