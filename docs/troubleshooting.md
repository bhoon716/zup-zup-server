...
## 16. 대표 시간표 설정 시 500 Internal Server Error 해결

### 문제 상황

사용자가 '대표 시간표'를 변경(PATCH /api/v1/timetables/primary)하려고 할 때, 간헐적으로 500 에러가 발생했습니다.

### 원인 분석

로그 분석 결과, `TimetableService` 내에서 대표 시간표 정보를 업데이트하는 과정에서 특정 조건(기존 대표 시간표 부재 등)에 대한 예외 처리가 미흡하거나, JPA 엔티티 상태 변경 시 발생하는 동시성 충돌이 문제의 원인이었습니다.

### 해결책

- **로깅 강화**: 에러 발생 지점에 상세 로그(`log.error`)를 추가하여 파라미터와 유저 상태를 추적했습니다.
- **로직 보강**: 기존 대표 시간표를 해제하고 새로운 대표 시간표를 설정하는 과정을 단일 트랜잭션 내에서 더욱 견고하게 처리하도록 리팩토링했습니다.

### 결과

대표 시간표 전환 기능이 안정화되었으며, 에러 발생 시 원인을 즉시 파악할 수 있는 로깅 기반을 마련했습니다.
